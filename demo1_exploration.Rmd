---
title: "Hardford Crime and Property Value Data Exploration"
output:
  html_document:
    df_print: paged
---
...
```{r}
<<<<<<< HEAD:demo1_exploration.Rmd
parcel_proj <- X |> st_transform(crs = 26956) |> st_centroid() |> 
  st_coordinates()
stealing_proj <- stealing |> st_transform(crs = 26956) |> 
  st_coordinates()
X$Thefts_kde <- kde(stealing_proj, Hscv(stealing_proj)) |> 
  predict(x = parcel_proj)
=======
parcel_proj <- X |> st_transform(crs = 26956) |> st_centroid() |> st_coordinates()
violence_proj <- violence |> st_transform(crs = 26956) |> st_coordinates()
X$Violence_kde <- kde(violence_proj, Hscv(violence_proj)) |> predict(x = parcel_proj)
>>>>>>> ebf6e6ac26cbc12edfdca777de6618ff51b95b43:code1_Exploration.Rmd
```


```{r plots, fig.width = 10, fig.height = 8}
X_plot_vars <- st_drop_geometry(X)[c("Thefts", "Violence", 
...
GGally::ggpairs(o, columns = c("StdResid", "logPrice", "sqrtViolence", 
                               "sqrtLiving_Area", "Bed", "Bath", "Thefts"), 
