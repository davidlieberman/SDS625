...
parcels <- st_read(".//data//parcel_hartford.geojson")
crime <- st_read(".//data//crime_hartford_2016_2021.geojson")
...
hurtin <- crime_dg[grep(
  "ASSAULT|HOMICIDE|SHOOTING",
  crime_dg$UCR_1_Category), 
]


parcels$thefts <- sapply(st_is_within_distance(
  parcels, stealin, dist = units::set_units(0.1, "miles")
), length)
parcels$owie <- sapply(st_is_within_distance(
  parcels, hurtin, dist = units::set_units(0.1, "miles")
), length)

sfh <- parcels[parcels$State_Use_Description == "ONE FAMILY",]
