...
X <- p[, c("Assessed_Total", "thefts", "violence", "Living_Area", 
...
ggpairs(X, columns = c(1:8), lower = list(continuous = "points")) +
...
We leave out `Thefts` since it is highly correlated with `Violence` but has a lower correlation with `Price` than `Violence` does. For similar reasons, we leave out `Effective_Area` and keep `Living_Area`. We fit one linear regression with both `Bed` and `Bath` and one with only `Bath`.

```{r models}
# Fit linear models with manually selected variables
